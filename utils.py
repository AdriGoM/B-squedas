...
class PriorityQueue():
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda node: node.path_cost) # Ordenamos por el coste de la ruta
...
class Subestimation():
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda node: node.path_cost + self.problem.h(node)) #Ordenamos por el coste de la ruta y a parte
        # por la heurística desde el nodo actual hasta el nodo final
